...
-- It handles existing data and ensures idempotent operations
...
-- ----------------------------------------------------------------------------
-- Spec requires (family_id, user_id) as composite primary key, not separate id
-- Also role should be 'owner', 'admin', or 'member'

-- First, backup existing data
CREATE TEMP TABLE family_members_backup AS 
SELECT * FROM public.family_members;

-- Drop the existing table (will cascade to dependent objects)
...
-- Restore data with role mapping (current 'admin' stays 'admin', default to 'member')
...
SELECT DISTINCT ON (family_id, user_id)
  ...
  CASE 
    ...
    ELSE 'member'
  END as role,
  COALESCE(joined_at, NOW()) as added_at
...
-- ----------------------------------------------------------------------------
-- Current table is photo_comments, spec requires comments
...
-- Restore comments with family_id from photos
INSERT INTO public.comments (id, photo_id, family_id, user_id, body, created_at, updated_at)
SELECT 
  c.id,
  c.photo_id,
  p.family_id,  -- Get family_id from photo
  c.user_id,
  c.comment as body,
  c.created_at,
  c.edited_at as updated_at
...
-- ============================================================================
-- PART 5: STORAGE POLICIES (if bucket exists)
-- ============================================================================

-- Create photos bucket if it doesn't exist (requires service role)
...
BEGIN
  ...
  IF EXISTS (
    ...
    DROP POLICY IF EXISTS "storage.select.members" ON storage.objects;
    DROP POLICY IF EXISTS "storage.insert.members" ON storage.objects;
    DROP POLICY IF EXISTS "storage.update.owner_or_admin" ON storage.objects;
    DROP POLICY IF EXISTS "storage.delete.owner_or_admin" ON storage.objects;
    
    -- Create new policies
    EXECUTE 'CREATE POLICY "storage.select.members" ON storage.objects
    FOR SELECT USING (
      bucket_id = ''photos'' AND
      EXISTS (
        SELECT 1 FROM public.family_members fm
        WHERE fm.user_id = auth.uid()
          AND fm.family_id = (split_part(objects.name, ''/'', 2))::uuid
      )
    )';
    
    EXECUTE 'CREATE POLICY "storage.insert.members" ON storage.objects
    FOR INSERT WITH CHECK (
      bucket_id = ''photos'' AND
      EXISTS (
        SELECT 1 FROM public.family_members fm
    ...
    EXECUTE 'CREATE POLICY "storage.update.owner_or_admin" ON storage.objects
    FOR UPDATE USING (
      bucket_id = ''photos'' AND
      EXISTS (
        SELECT 1 FROM public.family_members fm
    ...
    EXECUTE 'CREATE POLICY "storage.delete.owner_or_admin" ON storage.objects
    FOR DELETE USING (
      bucket_id = ''photos'' AND
      EXISTS (
        SELECT 1 FROM public.family_members fm
...
-- ============================================================================
-- PART 6: CLEAN UP OLD TABLES/COLUMNS
-- ============================================================================
